...
- The GUI still parses the potential field with `eval()` (high; arbitrary code execution).
...
- `README.md` and the preset descriptions overstate what several presets show, and a handful
  of documented numbers come from configs a reader cannot reproduce.
...
|---|---|---|---|---|---|
| 1 | The potential field is parsed with `eval()` under an empty-`__builtins__` guard, which is escapable. Threat: a V string pasted from an untrusted source. | **high** (code execution; one-line fix) | `gui/main_window.py:1701-1706` (field `:852`) | A harmless payload run through the exact same `eval(v_text, {"__builtins__":{}}, {...})` recovered the real builtins dict (160 names incl. `eval`, `exec`, `__import__`, `open`) via the standard `__subclasses__()` sandbox escape. Nothing was invoked. | `ast.literal_eval` parses all 13 presets' V strings identically, complex entries included, so it is a drop-in replacement. |
...
| 21 | Several preset descriptions overstate what the preset shows | low | `README.md:108-117,129-130`; `gui/main_window.py:410-433,452-453` | Strong Wall (V=3.5): "near-total reflection", but `4/(4+V²)=0.246` transmits (measured 0.244). Weak Barrier: reflection is ≤0.4% of the linear heatmap scale → LUT bin 0, drawn as zero. Double Barrier: cavity mass only ever falls, gone by t≈10 of a 65-unit run (each barrier reflects ~7%). Wide Barrier: sites are spaced (j=−3,−1,1,3), mid-band, 58% transmits — no tunneling. Slow Packet: v≈1.13 (56% of max), runs off the ±80 frame. Random Lattice: 5 fixed sites, 91% transmits as one packet. Balanced sigma splits into two packets moving apart, not a "standing" one. | Reword each description (or change the physics, e.g. V=10 for the wall, contiguous high-V barrier, E0≈−1.95 for the slow packet). |
